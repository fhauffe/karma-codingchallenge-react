...
class Attributes extends React.Component {
    ...
    showAttributePointsLeft() {
        alert(this.state.attributePoints);
    }

    increaseAttribute(attribute, number) {
        alert(this.state.buttonPlus);

        this.setState({ 
            attribute: this.state.attribute + number
        });
    }

    decreaseAttribute(attribute, number) {
        alert(this.state.buttonMinus);
        this.setState({ 
            attribute: this.state.attribute + number
        });
    }
        ...
        return (
            <Container>
                <Typography variant="H1">Verbleibende Punkte: {this.state.attributePoints}</Typography>
                    ...
                    <Skills attributePointsLeft={this.showAttributePointsLeft.bind(this)}></Skills>
          </Container>
        );
    }
}
